...
function printCountryData(countries){
    countries.forEach(currentCountry => {  // 29min24
        //console.log("hello");
        ...
        console.log("Name, population and percentage population of five biggest cities are: " );
        let sumPopulationBiggestCities = 0;
        currentCountry.largestCities.forEach((currentCity, index) => {
            let percentagePopulation = ((currentCity.population/currentCountry.population)*100).toFixed(2);
            console.log(
                `${index+1}.${currentCity.name}  ${currentCity.population}  ${percentagePopulation}%`);
            sumPopulationBiggestCities += currentCity.population;
                  
        });//on the left of the dot is object, on the right is method or property
        console.log("Sum of population in five biggest cities is:" + sumPopulationBiggestCities); 
